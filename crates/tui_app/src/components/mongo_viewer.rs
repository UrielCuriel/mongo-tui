...
impl MongoViewer {
    ...
    fn get_pane_shortcuts(&self, pane: ActivePane) -> Vec<(&'static str, &'static str)> {
        match pane {
            ActivePane::Connections => vec![
                ("c", "Add"),
                ("Enter", "Connect"),
                ("j/k", "Nav"),
                ("Del", "Remove"),
            ],
            ActivePane::Databases => vec![
                ("Enter", "Select/Expand"),
                ("j/k", "Nav"),
            ],
            ActivePane::Query => vec![
                ("Enter", "Edit"),
            ],
...
impl Component for MongoViewer {
    ...
    fn handle_key_events(&mut self, key: KeyEvent) -> Result<Option<Action>> {
        ...
        if let PopupState::JsonViewer(_json, _, offset) = &mut self.popup_state {
            match key.code {
                KeyCode::Esc => {
                    self.popup_state = PopupState::None;
                    return Ok(Some(Action::Render));
        ...
        if let PopupState::Help(state) = &mut self.popup_state {
             match key.code {
                ...
                KeyCode::Enter => {
                    ...
                    if !filter_str.trim().is_empty() {
                        if let Err(e) = serde_json::from_str::<serde_json::Value>(&filter_str) {
                            self.input_validation_errors
                                .insert(QueryField::Filter, format!("Invalid JSON: {}", e));
                            is_valid = false;
                        }
                    }
                    ...
                    if !sort_str.trim().is_empty() {
                        if let Err(e) = serde_json::from_str::<serde_json::Value>(&sort_str) {
                            self.input_validation_errors
                                .insert(QueryField::Sort, format!("Invalid JSON: {}", e));
                            is_valid = false;
                        }
                    }
                    ...
                    if !proj_str.trim().is_empty() {
                        if let Err(e) = serde_json::from_str::<serde_json::Value>(&proj_str) {
                            self.input_validation_errors
                                .insert(QueryField::Projection, format!("Invalid JSON: {}", e));
                            is_valid = false;
                        }
                    }
                    ...
                    if !limit_str.trim().is_empty() {
                        if limit_str.parse::<i64>().is_err() {
                            self.input_validation_errors
                                .insert(QueryField::Limit, "Must be a number".to_string());
                            is_valid = false;
                        }
                    }
        ...
        match key.code {
            ...
            KeyCode::Char('1') => {
                self.active_pane = ActivePane::Connections;
                return Ok(Some(Action::Render));
            }
            KeyCode::Char('2') => {
            ...
            KeyCode::Char('3') => {
                self.active_pane = ActivePane::Query;
                return Ok(Some(Action::Render));
            }
            KeyCode::Char('4') => {
                self.active_pane = ActivePane::Documents;
                return Ok(Some(Action::Render));
            }
            KeyCode::Char('?') => {
            ...
            _ => {
                match self.active_pane {
                    ActivePane::Connections => match key.code {
                    ...
                    ActivePane::Documents => {
                        match key.code {
                            ...
                            KeyCode::Right | KeyCode::Char('l')
                                if self.view_mode == ViewMode::Table =>
                            {
                                if self.selected_column_index
                                    < self.visible_fields.len().saturating_sub(1)
                                {
                                    self.selected_column_index += 1;
                                    return Ok(Some(Action::Render));
                                }
                            ...
                            KeyCode::Char('p') if self.view_mode == ViewMode::Table => {
                                if let Some(idx) = self.document_table_state.selected() {
                                    if let Some(doc) = self.documents.get(idx) {
                                        if let Some(field) =
                                            self.visible_fields.get(self.selected_column_index)
                                        {
                                            let val = doc
                                                .get(field)
                                                .map(|v| v.to_string())
                                ...
                                if let Some(idx) = selected_idx {
                                    if let Some(doc) = self.documents.get(idx) {
                                        if let Ok(json) = serde_json::to_string_pretty(doc) {
                                            ...
                                            self.popup_state =
                                                PopupState::JsonViewer(json, id_str, 0);
                                            return Ok(Some(Action::Render));
                                        }
                                    }
                                }
                    ...
                    ActivePane::Query => {
                        match key.code {
                            KeyCode::Enter => {
                                // Open Query Builder
                                self.popup_state = PopupState::QueryBuilder {
                                    active_field: QueryField::Filter,
                                };
                                return Ok(Some(Action::Render));
                            }
                            _ => {}
        ...
        if let Some(action_tx) = &self.action_tx {
            ...
            match action {
                ...
                Action::DatabasesLoaded(dbs) => {
                    ...
                    self.active_pane = ActivePane::Databases;
                    self.rebuild_tree_items();
                    self.selected_tree_index = if !self.tree_items.is_empty() {
                        Some(0)
                    } else {
                        None
                    };
                ...
                Action::RefreshDocuments => {
                    if let (Some(db_idx), Some(coll_idx)) =
                        (self.selected_db_index, self.selected_coll_index)
                    {
                        if let Some(db) = self.databases.get(db_idx) {
                            if let Some(coll) = db.collections.get(coll_idx) {
                                ...
                                let filter_str = self.query_input.lines().join("\n");
                                let filter: Option<mongo_core::bson::Document> =
                                    if !filter_str.trim().is_empty() {
                                ...
                                let projection: Option<mongo_core::bson::Document> =
                                    if !projection_str.trim().is_empty() {
                                ...
                                let sort_str = self.sort_input.lines().join("\n");
                                let sort: Option<mongo_core::bson::Document> =
                                    if !sort_str.trim().is_empty() {
                                ...
                                tokio::spawn(async move {
                                    match mongo_core
                                        .find_documents(
                                            &db_name, &coll_name, filter, projection, sort, limit,
                                            skip,
                                        )
                                        .await
                                    {
                ...
                Action::DocumentsLoaded(docs) => {
                    ...
                    // Reset selection
                    self.document_table_state
                        .select(if !self.documents.is_empty() {
                            Some(0)
                        } else {
                            None
                        });
    ...
    fn draw(&mut self, f: &mut Frame, area: Rect) -> Result<()> {
        ...
        match &mut popup {
            PopupState::JsonViewer(json, id, offset) => {
                self.draw_json_popup(f, area, json, id, *offset)
            }
            PopupState::ConnectionManager {
                name,
                uri,
                is_editing_uri,
...
impl MongoViewer {
    fn draw_sidebar(&self, f: &mut Frame, area: Rect) {
        ...
        // Connections List
        let conn_shortcuts = if self.active_pane == ActivePane::Connections {
            self.get_pane_shortcuts(ActivePane::Connections)
        } else {
            vec![]
        };
        let conn_shortcuts_str = conn_shortcuts
            .iter()
            .map(|(k, v)| format!("{}: {}", k, v))
            .collect::<Vec<_>>()
            .join(" | ");
        ...
        let conn_block = Block::default()
            .title("[1] Connections")
            .title_bottom(Line::from(conn_shortcuts_str).alignment(Alignment::Center))
            .borders(Borders::ALL)
            .border_type(BorderType::Rounded)
        ...
        let conn_items: Vec<ListItem> = self
            .connections
            .iter()
        ...
        // Databases Tree
        let db_shortcuts = if self.active_pane == ActivePane::Databases {
            self.get_pane_shortcuts(ActivePane::Databases)
        } else {
            vec![]
        };
        let db_shortcuts_str = db_shortcuts
            .iter()
            .map(|(k, v)| format!("{}: {}", k, v))
            .collect::<Vec<_>>()
            .join(" | ");
        ...
        let tree_block = Block::default()
            .title("[2] Databases")
            .title_bottom(Line::from(db_shortcuts_str).alignment(Alignment::Center))
            .borders(Borders::ALL)
            .border_type(BorderType::Rounded)
        ...
        let tree_items_widgets: Vec<ListItem> = self
            .tree_items
            .iter()
            .map(|item| match item {
    ...
    fn draw_query_bar(&self, f: &mut Frame, area: Rect) {
        let shortcuts = if self.active_pane == ActivePane::Query {
            self.get_pane_shortcuts(ActivePane::Query)
        } else {
            vec![]
        };
    ...
    fn draw_documents(&mut self, f: &mut Frame, area: Rect) {
        if self.selected_coll_index.is_none() {
            ...
            let block = Block::default()
                ...
                .title("[4] Documents");
            f.render_widget(
                Paragraph::new("Select a collection to view documents.").block(block),
                area,
            );
            return;
        }
        ...
        let coll_name = if let (Some(db_idx), Some(coll_idx)) =
            (self.selected_db_index, self.selected_coll_index)
        {
            self.databases
                .get(db_idx)
        ...
        let breadcrumb = format!(" [4] {} / {} / {} ", conn_name, db_name, coll_name);

        // View Mode
        let view_mode_str = if self.view_mode == ViewMode::Table {
            " Table "
        } else {
            " JSON "
        };
        ...
        let shortcuts_str = if self.active_pane == ActivePane::Documents && has_selection {
            self.get_pane_shortcuts(ActivePane::Documents)
                .iter()
                .map(|(k, v)| format!("{}: {}", k, v))
                .collect::<Vec<_>>()
                .join(" | ")
        } else if self.active_pane == ActivePane::Documents {
            "j/k: Nav".to_string()
        } else {
             String::new()
        };
        ...
        if self.view_mode == ViewMode::Table {
            let header_cells = self.visible_fields.iter().enumerate().map(|(i, h)| {
                let style = if i == self.selected_column_index
                    && self.active_pane == ActivePane::Documents
                {
                    Style::default()
                        .fg(Color::Red)
    ...
    fn draw_help_popup(&self, f: &mut Frame, area: Rect, state: &mut TableState) {
        ...
        // Global
        rows.push(Row::new(vec![Cell::from("GLOBAL").style(Style::default().add_modifier(Modifier::BOLD))]));
        for (k, v) in self.get_global_shortcuts() {
             rows.push(Row::new(vec![Cell::from(k), Cell::from(v)]));
        }
        rows.push(Row::new(vec![Cell::from("")])); // Spacer

        // Sections
        let sections = vec![
            (ActivePane::Connections, "CONNECTIONS"),
            (ActivePane::Databases, "DATABASES"),
            (ActivePane::Query, "QUERY"),
            (ActivePane::Documents, "DOCUMENTS"),
        ];

        for (pane, title) in sections {
             rows.push(Row::new(vec![Cell::from(title).style(Style::default().add_modifier(Modifier::BOLD))]));
             for (k, v) in self.get_pane_shortcuts(pane) {
                 rows.push(Row::new(vec![Cell::from(k), Cell::from(v)]));
             }
             rows.push(Row::new(vec![Cell::from("")])); // Spacer
        }
    ...
    fn draw_connection_manager_popup(
        &self,
        f: &mut Frame,
        area: Rect,
        name: &TextArea<'static>,
        ...
        name_widget.set_block(
            Block::default()
                .borders(Borders::ALL)
                .title("Name")
                .border_style(if !is_editing_uri {
                    Style::default().fg(Color::Yellow)
                } else {
                    Style::default()
                }),
